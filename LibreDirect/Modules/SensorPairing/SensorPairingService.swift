                                        ...
                                        self.readRaw(0x1A00, 64, tag: tag) { _, _, _ in
                                            self.readRaw(0xFFAC, 36, tag: tag) { _, _, _ in
                                                ...
                                                tag.customCommand(requestFlags: .highDataRate, customCommandCode: Int(cmd.code), customRequestParameters: cmd.parameters) { response, error in
                                                    ...
                                                    let decryptedFram = PreLibre.decryptFRAM(sensorUID: sensorUID, patchInfo: patchInfo, fram: fram)
                                                    if let decryptedFram = decryptedFram {
                                                        self.readingsSubject.send(SensorPairingInfo(uuid: sensorUID, patchInfo: patchInfo, fram: decryptedFram, streamingEnabled: streamingEnabled))
                                                    } else {
                                                        self.readingsSubject.send(SensorPairingInfo(uuid: sensorUID, patchInfo: patchInfo, fram: fram, streamingEnabled: streamingEnabled))
                                                    }
