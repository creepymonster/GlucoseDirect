...
struct GlucoseChartView: View {
    ...
    private enum Config {
        ...
        static let alarmStrokeStyle = StrokeStyle(lineWidth: 0.2)
        ...
        static let targetStrokeStyle = StrokeStyle(lineWidth: 0.2)
    ...
    private func updateHelpVariables(fullSize: CGSize, glucoseValues: [SensorGlucose]) {
        if let first = glucoseValues.first, let last = glucoseValues.last {
            let firstTimeStamp = first.timeStamp.addingTimeInterval(-1 * 5 * 60)
            let lastTimeStamp = last.timeStamp.addingTimeInterval(5 * 60)
    ...
    private func updateXGrid(fullSize: CGSize, firstTimeStamp: Date?, lastTimeStamp: Date?) {
        calculationQueue.async {
            if let firstTimeStamp = firstTimeStamp, let lastTimeStamp = lastTimeStamp {
                let allHours = Date.dates(from: firstTimeStamp.rounded(on: 15, .minute).addingTimeInterval(-1 * 30 * 60), to: lastTimeStamp.addingTimeInterval(30 * 60))
                ...
                for hour in allHours {
                    ...
                    xGridTexts.append(TextInfo(description: hour.localTime, x: x, y: y))
                }
    ...
    private func updateYGrid(fullSize: CGSize, alarmLow: Int?, alarmHigh: Int?, targetValue: Int?, glucoseUnit: GlucoseUnit) {
        calculationQueue.async {
            ...
            for i in gridParts {
                ...
                let y = self.translateGlucoseToY(fullSize: fullSize, glucose: CGFloat(i))
                yGridTexts.append(TextInfo(description: i.asGlucose(unit: glucoseUnit), x: 0, y: y))
            }
